...
#define SHADER_COLOR_INDEX	3
	...
	struct Renderer2DStorage
	{
		...
		std::vector<const glm::mat4> TransformationStack;
	...
	void BatchRenderer2D::Submit(const Ref<UIElement>& element)
	{
		...
		s_Data->Buffer->vertex = glm::translate(*(s_Data->TransformationBack), pos);
		s_Data->Buffer->uv = uv[0];
		s_Data->Buffer->Tid = ts;
		s_Data->Buffer->color = color;
		s_Data->Buffer++;

		s_Data->Buffer->vertex = glm::translate(*(s_Data->TransformationBack), glm::vec3(pos.x, pos.y + size.y, pos.z));
		s_Data->Buffer->uv = uv[1];
		s_Data->Buffer->Tid = ts;
		s_Data->Buffer->color = color;
		s_Data->Buffer++;

		s_Data->Buffer->vertex = glm::translate(*(s_Data->TransformationBack), glm::vec3(pos.x + size.x, pos.y + size.y, pos.z));
		s_Data->Buffer->uv = uv[2];
		s_Data->Buffer->Tid = ts;
		s_Data->Buffer->color = color;
		s_Data->Buffer++;

		s_Data->Buffer->vertex = glm::translate(*(s_Data->TransformationBack), glm::vec3(pos.x + size.x, pos.y, pos.z));
	...
	void BatchRenderer2D::DrawString(const Ref<Font>& font, const std::string& text, const glm::vec3& position, const glm::vec4& color)
	{
		float ts = 0.0f;
		bool found = false;
		for (int i = 0; i < s_Data->TextureSlots.size(); i++)
		...
		for (int i = 0; i < text.length(); i++)
		{
			...
			if (glyph != NULL)
			{
				...
				s_Data->Buffer->vertex = glm::translate(*(s_Data->TransformationBack), glm::vec3(x0, y0, 0.0f));
				s_Data->Buffer->uv = glm::vec2(u0, v0);
				s_Data->Buffer->tid = ts;
				s_Data->Buffer->color = color;
				s_Data->Buffer++;

				s_Data->Buffer->vertex = glm::translate(*(s_Data->TransformationBack), glm::vec3(x0, y1, 0.0f));
				s_Data->Buffer->uv = glm::vec2(u0, v1);
				s_Data->Buffer->tid = ts;
				s_Data->Buffer->color = color;
				s_Data->Buffer++;
				
				s_Data->Buffer->vertex = glm::translate(*(s_Data->TransformationBack), glm::vec3(x1, y1, 0));
				s_Data->Buffer->uv = glm::vec2(u1, v1);
				s_Data->Buffer->tid = ts;
				s_Data->Buffer->color = color;
				s_Data->Buffer++;
				
				s_Data->Buffer->vertex = glm::translate(*(s_Data->TransformationBack), glm::vec3(x1, y0, 0));
